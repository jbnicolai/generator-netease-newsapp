...
Deploy.prototype.uploadPage = function(app,type){
  ...
  function basePromise(){
    ...
    self.client.mkdir(ftpConfig[type].urlBase+'/'+app+'/views',true,function(e){
      var dirs = [ftpConfig[type].fileDir+'/',ftpConfig[type].fileDir+'/views/'].map(function(d,i){
        ...
        fs.readdir(d, function (err, files) {
          var htmls = files.filter(function (h, i) {
            ...
            var deferred = q.defer();
            self.client.put(d + v, ftpConfig[type].urlBase + '/' + app + '/' + d.substring(ftpConfig[type].fileDir.length + 1) + v, false, function (e) {
              deferred.resolve();
            });
            return deferred.promise;
          });
